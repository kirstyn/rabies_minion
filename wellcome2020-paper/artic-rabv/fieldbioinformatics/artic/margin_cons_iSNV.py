...
def report(r, status, allele):
    ...
    print("%s\t%s\tbasecalledfrac\t%s" % (idfile, r.POS, r.INFO.get('BaseCalledFrequency', ['n/a'])), file=sys.stderr)
...
def main():
    ...
    for record in vcf_reader:
        if record.ALT[0] != '.':
            ...
            basecall = float(record.INFO['BaseCalledFrequency'][0])
            qual = record.QUAL
            ...
            if qual >= 200 and total_reads >= 20 and basecall < 0.5 and support < 0.75:
                HET=''.join(sorted(REF+ALT))
                d = {'AC': 'M', 'AG': 'R', 'AT': 'W', 'CG': 'S', 'CT': 'Y', 'GT': 'K', 'ACG': 'V', 'ACT': 'H', 'AGT': 'D', 'CGT': 'B', 'ACGT': 'N'}
                HET=d[HET]
                #print >>sys.stderr, REF, HET
                report(record, "variant", HET)
                sett.add(record.POS)
                cons[record.POS-1] = str(HET)
                continue
