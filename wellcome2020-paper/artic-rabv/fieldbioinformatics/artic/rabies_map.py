#Kirstyn Brunker: modified artic minion.py (written by Nick Loman)
...
def run(parser, args):
    ...
    cmds.append("minimap2 -ax map-ont -t %s %s %s | samtools view -bS - | samtools sort -o %s.sorted.bam -" % (args.threads, ref, read_file, args.sample))
